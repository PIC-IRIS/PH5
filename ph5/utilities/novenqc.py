...
class Novenqc():
    ...
    def qc_map(self, outfile):
        ...
        if 'channel_number_i' in self.NAMES:
            is_rec = True
        else:
            is_rec = False

        varrays = sorted(self.TOP.keys())
        mmap = kml.Kml()
        if is_rec:
            for varray in varrays:
                # receiver
                vids = sorted(self.TOP[varray].keys())
                for vid in vids:
                    chans = self.TOP[varray][vid].keys()
                    for chan in chans:
                        rows = self.TOP[varray][vid][chan]
                        for r in rows:
                            llz = get_lat_lon(r)
            ...
            for varray in varrays:
                # event
                vids = self.TOP[varray].keys()
                for vid in vids:
                    rows = self.TOP[varray][vid]
                    for r in rows:
                        llz = get_lat_lon(r)
                        des = ""
    ...
    def mk_table(self, table, sep):
        ...
        for t in table:
            ...
            try:
                ...
                errs.append(
                    "Exception {0} at input file line {1}.".
                    format(e.message, n))
    ...
    def qc_deploy_pickup(self, rows):
        '''
        ...
        if dist is not None and dist > 5:
            # XXX   ERROR   XXX
            errs.append(
                "{0}-{1} Warning: Distance between deployment and pickup "
                "locations seems large for station {2}.".format(
        ...
        if time_delta > 15552000 or time_delta < 0:
            errs.append(
                "{0}-{1} Warning: time between deployment and pickup unusual "
                "for station {2}.".format(
                    line_numbers[0], line_numbers[1], r['id_s']))

        return errs

    def qc_bulogic_receiver(self):
        '''
        ...
        for varray in varrays:
            vids = sorted(self.TOP[varray].keys())
            for vid in vids:
                if isinstance(self.TOP[varray][vid], dict):
                    chans = sorted(self.TOP[varray][vid].keys())
                    for chan in chans:
                        rows = self.TOP[varray][vid][chan]
                        err = self.qc_deploy_pickup(rows)
                        if err:
                            ret += err

        return ret

    def match_range(self, value, rrange):
        '''
    ...
    def match_re(self, value, reg_expr):
        '''
    ...
    def match_type(self, value, ttype):
        '''
           Check to see if value matches type, example int15, string1024,
           float64.
        '''
        if ttype is None:
            return True
        ...
        for t in self.TABLE:
            ...
            for i in range(len(t)):
                ...
                if key == 'Ignore':
                    continue
                v = t[i]
                if 'type' in self.COLS[key]:
                    ttype = self.COLS[key]['type']
                else:
                    ttype = None
                if 'help' in self.COLS[key]:
                    hhelp = self.COLS[key]['help']
                else:
                    hhelp = None
                if 're' in self.COLS[key]:
                    rre = self.COLS[key]['re']
                else:
                    rre = None
                if 'range' in self.COLS[key]:
                    rrange = self.COLS[key]['range']
                else:
                    rrange = None
                # Check regular expression
                if not self.match_re(v, rre):
                    ret.append(
                        "{0}: Value of column {1} {2} does not match re. Help:"
                        " {3}".format(
                            n, key, v, hhelp))
                # Check type
                if not self.match_type(v, ttype):
                    ret.append(
                        "{0}: Value of column {1} {2} does not match type. "
                        "Type: {3}".format(
                            n, key, v, ttype))
                # Check range
                if not self.match_range(v, rrange):
                    ret.append(
    ...
    def qc_req_cols(self):
        '''
           Check to see if all required columns are present.
        '''
        ret = []
        cols_keys = self.COLS.keys()
        for k in cols_keys:
            info_keys = self.COLS[k]
            ...
            if req is True:
                ...
                flds = stack()
                print "Should never get here: {0} {1}.".\
                      format(flds[1], flds[2])

        return ret

    def qc_receivers(self, table, names, cols, sep=','):
        '''
        ...
        ret = self.qc_req_cols()
        # Convert to list (rows) of lists ()
        self.TABLE, err = self.mk_table(table, sep)
        if err != [] or ret != []:
        ...
        self.TOP = self.churn()
        self.ERR = self.qc_bulogic_receiver()

        return False

    def qc_shots(self, table, names, cols, sep=','):
        '''
