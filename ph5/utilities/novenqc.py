...
def qc_deploy_pickup(rows):
    ...
    if dist is not None and dist > 5:
        #   XXX   ERROR   XXX
        errs.append("{0}-{1} Warning: Distance between deployment and pickup locations seems large for station {2}.".format(
    ...
    if time_delta > 15552000 or time_delta < 0:
        errs.append("{0}-{1} Warning: time between deployment and pickup unusual for station {2}.".format(
            line_numbers[0], line_numbers[1], r['id_s']))
    ...
    for t in TABLE:
        ...
        for i in range(len(t)):
            ...
            if not match_re(v, rre):
                ret.append("{0}: Value of column {1} {2} does not match re. Help: {3}".format(
                    n, key, v, hhelp))
            #   Check type
            if not match_type(v, ttype):
                ret.append("{0}: Value of column {1} {2} does not match type. Type: {3}".format(
                    n, key, v, ttype))
            #   Check range
            if not match_range(v, rrange):
                ret.append("{0}: Value of column {1} {2} does not match expected range. Range: {3}".format(
                    n, key, v, rrange))
            #   Check if ascii time
            if timeRE.match(key):
                try:
                    ...
                    try:
                        ...
                        ret.append(
                            "{0}: Value of column {1} {2} does not match expected time string".format(n, key, v))
