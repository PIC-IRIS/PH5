...
class Set_n_i_Response():
    def get_args(self):
        ...
        parser.add_argument(
            "-F", dest="families_directory", required=False,
            help="Directory that holds the family directories. Absolute path.")
        parser.add_argument(
            "-N", dest="first_n_i", required=False, type=int, default=0,
            help="The n_i of the first entry. Defaults to zero.")

        self.ARGS = parser.parse_args()

        if self.ARGS.families_directory is None:
            self.ARGS.families_directory = os.getcwd()

    def dump_kefs(self):
        '''   Dump each family Response_t into a single kef file.
              Label start of each Response_t with the family name
              '## A' as example
        '''
        ...
        self.ORIG_RESPS = os.path.join(
            here, "RESPONSE_T_N_I", "Response_t_by_family.kef")
        LOGGER.info("Creating: {0}".format(self.ORIG_RESPS))

        with open(self.ORIG_RESPS, 'w+') as fh:
    ...
    def parse_kef(self):
        ...
        row = None
        n_i_all = 0
        with open(self.ORIG_RESPS) as fh:
            while True:
                line = fh.readline()
                if not line:
                    break
                ...
                # We start a new row in the table here
                    ...
                    row = {}
                    row['n_i_all'] = n_i_all + self.ARGS.first_n_i
            ...
            row['n_i_family'] = last_n_i + self.ARGS.first_n_i
            ret[family].append(row)
        return ret

    def print_new_Response_t(self, n_i_map):
        '''   Print the final Response_t.kef
        '''
        self.CORRECTED_RESPS = os.path.join(
            self.ARGS.families_directory,
            "RESPONSE_T_N_I", "Response_t_cor.kef")
        LOGGER.info("Creating: {0}".format(self.CORRECTED_RESPS))
        with open(self.CORRECTED_RESPS, 'w+') as fh:
    ...
    def print_new_Das_t(self, P5, n_i_map, family):
        ...
        for das_g in P5.Das_g_names:
            ...
            if not os.path.exists(os.path.join(
                    self.ARGS.families_directory, "RESPONSE_T_N_I")):
...
def main():
    set_ni = Set_n_i_Response()
    set_ni.get_args()
    os.chdir(set_ni.ARGS.families_directory)
    set_ni.dump_kefs()
    try:
        n_i_map = set_ni.parse_kef()
    except BaseException:
        LOGGER.error("Cannot create n_i map. "
                     "Make sure the directory is correct using -F flag")
    else:
        for family in ALL_FAMILIES:
            ph5 = os.path.join(set_ni.ARGS.families_directory, family)
            try:
            ...
            set_ni.print_new_Das_t(P5, n_i_map, family)
            P5.close()
        set_ni.print_new_Response_t(n_i_map)
