...
def check_lat_lon_elev(station, warnings):
    ...
    if station['location/X/value_d'] in [None, '']:
        warnings.append("No Channel longitude value found.")
    else:
        try:
            if not -180 <= float(station['location/X/value_d']) <= 180:
                warnings.append("Channel longitude %s not in range [-180,180]"
                                % station['location/X/value_d'])
        except ValueError:
            warnings.append("Channel longitude %s is not a number."
                            % station['location/X/value_d'])
    ...
    if station['location/Y/value_d'] in [None, '']:
        warnings.append("No Channel latitude value found.")
    else:
        try:
            if not -90 <= float(station['location/Y/value_d']) <= 90:
                warnings.append("Channel latitude %s not in range [-90,90]"
                                % station['location/Y/value_d'])
        except ValueError:
            warnings.append("Channel latitude %s is not a number."
                            % station['location/Y/value_d'])
    ...
    if station['location/Z/value_d'] in [None, '']:
        warnings.append("No Channel location/Z/value_d value found.")
