...
class SortKefGen():
    ...
    def get_args(self):
        ...
        parser.usage = (
            "sort_kef_gen --nickname ph5-file-prefix --serial-number "
        ...
        parser.add_argument(
            "-a", "--auto", dest="auto", action="store_true", default=False,
            help=("Attempt to auto detect windows. Windows "
                  "should start at the same time on all DASs."))
        ...
        if self.SN is None and self.AUTO is False:
            raise Exception(
                "Serial number can not be undefined if auto is set to false.")
    ...
    def get_sample_count(self, g, a):
        self.EX.ph5_g_receivers.setcurrent(g)

        try:
            node = self.EX.ph5_g_receivers.find_trace_ref(a)
    ...
    def read_das_table(self, das):
        ...
        das_group = self.EX.ph5_g_receivers.getdas_g(das)

        if das_group is None:
            return False

        self.EX.ph5_g_receivers.setcurrent(das_group)

        r, k = self.EX.ph5_g_receivers.read_das()
        ...
        k.append('samples')
        self.DAS_T = Rows_Keys(R, k)

        return True

    def read_all_das(self):
    ...
    def report_gen(self):
        ...
        PH5_VERSION = self.EX.version()

        ar = self.EX.ph5_g_sorts.names()
        ...
        now = time.time()
        self.print_report("#   sort-kef-gen Version: %s ph5 Version: %s"
                          % (PROG_VERSION, PH5_VERSION))
        r = 1
        # XXX   This assumes that the arrays were deployed for the same
        # recording windows.   XXX
        for a in ar:
            array_t, k = self.EX.ph5_g_sorts.read_arrays(a)
            array_deploy, array_pickup = self.first_last(array_t)
            if len(self.DAS_T.rows) < 1:
                LOGGER.warning("Failed to read any DAS information!\n")

            for d in self.DAS_T.rows:
                ...
                if array_deploy <= d['time/epoch_l'] \
                   and array_pickup >= int_part:

                    self.print_report(
                        "#   row {0} das {1}\n/Experiment_g/Sorts_g/Sort_t"
                        .format(r, d['das']))
                    self.print_report("\tarray_name_s = %s" % a[-3:])
                    self.print_report("\tarray_t_name_s = %s" % a)
                    self.print_report("\tdescription_s = Recording window %04d"
                                      % d['event_number_i'])
                    self.print_report(
                        "\tstart_time/epoch_l = %d" % d['time/epoch_l'])
                    self.print_report("\tstart_time/micro_seconds_i = %d"
                                      % d['time/micro_seconds_i'])
                    self.print_report(
                        "\tstart_time/type_s = %s" % d['time/type_s'])
                    self.print_report(
                        "\tstart_time/ascii_s = %s" % d['time/ascii_s'])
                    self.print_report("\tend_time/epoch_l = %d" % int_part)
                    self.print_report("\tend_time/micro_seconds_i = %d"
                                      % (float_part * 1000000.0))
                    self.print_report(
                        "\tend_time/ascii_s = %s" % time.ctime(t1))
                    self.print_report("\tend_time/type_s = BOTH")
                    self.print_report("\ttime_stamp/epoch_l = %d" % now)
                    self.print_report(
                        "\ttime_stamp/ascii_s = %s" % time.ctime(now))
                    self.print_report("\ttime_stamp/micro_seconds_i = 0")
                    self.print_report("\ttime_stamp/type_s = BOTH")
                    r += 1
...
def main():
    ...
    if sk.SN is not None:
        if not sk.read_das_table(sk.SN):
            LOGGER.error("Failed to read Das_t for {0}.".format(sk.SN))
            return 1
    elif sk.AUTO is True:
        if not sk.read_all_das():
