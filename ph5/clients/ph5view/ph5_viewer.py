...
PH5VALFILES = [path.join(mkdtemp(), 'PH5VAL%s.dat' % ch)
               for ch in range(3)]    # to keep PH5 values for reuse
...
class InfoPanel(QtGui.QFrame):
    def __init__(self, control, txt, canvas, statId):
        ...
        for chIndex in range(3):
            ch = chIndex + 1
            if ch not in self.control.channels or ch not in self.control.metadata[
                    self.statId]['chans']:
...
class SelectedStation(QtGui.QWidget):
    def __init__(self, parent, showPos=False):
        ...
        # self.setAttribute(Qt.Qt.WA_NoSystemBackground)  # transparent to see
        # the selected part under the widget
        self.setWindowFlags(QtCore.Qt.FramelessWindowHint)
        self.setAttribute(QtCore.Qt.WA_TranslucentBackground)
        self.setStyleSheet("background-color:transparent;")
        # self.setWindowFlags(QtCore.Qt.ToolTip)          # hide title bar of
        # the widget
...
class PrintSaveParaDialog(QtGui.QDialog):
    def __init__(self, parent, typeStr, unitStr, defaultSize):
        ...
        # allow to select option of showing legend in saved graphic or printed
        # graphic
...
class Canvas(app.Canvas):
    def __init__(self, parent, control):
        ...
        # the more text is used, the slower the program is => limit to show 25
        # texts at a time
        """
        self.text=[]
        for i in range (30):
    ...
    def initData(self, t=[], val=None, deepRemoving=False):
        ...
        for ch in range(len(self.control.channels)):
            if ch < len(self.control.conf['plotColor']):
                colors.append([QColor(c).getRgbF()[:3]
        ...
        if not deepRemoving:
            self.parent.canvScaleT, self.parent.canvScaleV = (
                1., 1.)    # for resetting pan/scale
            self.parent.panT, self.parent.panV = (0., 0.)
            # for real scaling
            self.canvScaleT, self.canvScaleV = (1, 1)
            self.panT, self.panV = (0., 0.)
        if val is not None:
            # this operation to change the staLimitList to range (-1,1): to
            # match with change in value
            self.control.statLimitList = 2 * \
                self.control.statLimitList / self.control.maxVal - 1

        self.parent.setWindowTitle('Main Window:  %s %s' % (
            ...
            for ch in self.control.channels:
                ...
                for i in range(len(val[ch])):
                    ...
                    if self.control.metadata[i] is None \
                        ...
                        self.data[ch][i]['a_position'][:, 0] = direct * \
                            t[self.control.keepList[ch][i]]
                        # change val to range (-1,1) - can't change in createVal()
                        # bc val is a list of nparray, not an nparray
                        self.data[ch][i]['a_position'][:,
                                                       1] = val[ch][i] * 2. / self.control.maxVal - 1
                        # an np array of all 0s for each data will be fed
                        # separately
        ...
        for ch in self.control.channels:
            ...
            for i in range(len(self.data[ch])):
                ...
                if i in self.control.PH5Info['quickRemoved'][ch].keys():
                    ...
                    colorIndex = i % len(
                        colors[self.control.channels.index(ch)])

                    c = colors[self.control.channels.index(ch)][colorIndex]

                self.data[ch][i]['a_color'] = np.tile(c, (aSize, 1))

        if 'showAbnormalStat' in self.control.conf and self.control.conf['showAbnormalStat']:
            for ch in self.control.channels:
                for abn in self.control.PH5Info['abnormal']:
                    aSize = len(self.data[ch][abn]['a_index'])
                    abColor = QColor(
                        self.control.conf['abnormalColor']).getRgbF()[:3]
                    self.data[ch][abn]['a_color'] = np.tile(
                        abColor, (aSize, 1))
        # feedData() and feedGData() separately for buildGrid() require info
        # created in feedData(): canvScaleT
    ...
    def initSupportData(self, mainCanvas, LT, RB):
        ...
        start = time.time()
        -1 if self.control.upRbtn.isChecked() else 1
        ...
        self.mainMinY, self.mainMaxY = self.getMinMaxY(
            self.LT, self.RB)    # to define lim in self.painting
        self.parent.canvScaleT, self.parent.canvScaleV = (
            self.canvScaleT, self.canvScaleV)  # for resetting pan/scale
        self.parent.panT, self.parent.panV = (
            self.panT, self.panV)         # for resetting
        self.feedData(self.panT, self.panV, self.canvScaleT, self.canvScaleV)
        self.gtData, self.gdData, self.timeY, self.tLabels, self.dLabels = self.buildGrid()
        self.feedGData(self.panT, self.panV, self.canvScaleT, self.canvScaleV)
        self.labelPos = self.resiteLabels()
        ...
        self.parent.setWindowTitle('Support Window: %s %s' % (
            self.PH5View.graphName, self.control.conf['addingInfo']))
    ...
    def feedData(self, tPan=0., vPan=0., tScale=1., vScale=1.):
        ...
        for ch in self.data.keys():
            ...
            for i in range(len(self.data[ch])):
                ...
                # time,val    (y,x) b/c the model has been turned 90 degree
                self.program[ch][i]['u_pan'] = (tPan, vPan)
                self.program[ch][i]['u_scale'] = (
                    tScale, vScale)      # time, val
    ...
    # set new self.needLablNo for self.drawing know how many labels need to be
    # drawn
    def resiteLabels(self, panT=None, panV=None,
        ...
        # w/ all fed data in range -1,1, scale of the drawing will affect
        # labels' position as well
        labelPos = []
        # check to see if labels have to skip any grids
        F = 1
        numOfLabels = 30
        while True:
            numOfLabels = math.ceil(
                self.control.totalTime) / (F * self.control.timeGridIntervalSB.value() * 1000 * canvScaleT)
            if numOfLabels <= 25:
                break
            F += 1
        ...
        if self.control.upRbtn.isChecked():
            ...
            try:
                while y > 0:
                    ...
                    labelPos.insert(
                        0, {'t': self.timeY[z + k], 'y': y + self.offsetY, 'text': "%s" % self.tLabels[z + k]})
                    k += F
                ...
                while y < self.height and z - k > 0:
                    y = self.height - \
                        int(0.5 * self.height *
                            ((self.timeY[z - k] - panT) * canvScaleT + 1))
                    labelPos.append(
                        {'t': self.timeY[z - k], 'y': y + self.offsetY, 'text': "%s" % self.tLabels[z - k]})
                    k += F
            except Exception as e:
                #print "2:" + str(e)
                pass
        ...
        while True:
            F += 1
            numOfDLabels = int(
                self.control.totalSize / (F * self.control.distanceGridIntervalSB.value() * 1000 * canvScaleV))
            (self.control.totalSize, F *
             self.control.distanceGridIntervalSB.value() * 1000, F, numOfDLabels)
        ...
        try:
            while x > 0 and z - k > 0:
                ...
                labelPos.insert(
                    0, {'d': self.dLabels[z - k][1], 'x': x + self.offsetX, 'text': "%s" % self.dLabels[z - k][0]})
                k += F
        except Exception as e:
            #print "5:"+str(e)
            pass
        ...
        try:
            while x < self.width:
                ...
                x = int(0.5 * self.width *
                        ((self.dLabels[z + k][1] + panV) * canvScaleV + 1))
                labelPos.append(
                    {'d': self.dLabels[z + k][1], 'x': x + self.offsetX, 'text': "%s" % self.dLabels[z + k][0]})
                k += F
    ...
    def buildGrid(self):
        ...
        gtData['a_position'][:, 0] = np.repeat(
            direct * timeY, 2)   # 2: each grid line need 2 points
        ...
        gtData['a_color'][self.zeroTIndex *
                          2] = QColor(QtCore.Qt.blue).getRgbF()[:3]
        gtData['a_color'][self.zeroTIndex * 2 +
                          1] = QColor(QtCore.Qt.blue).getRgbF()[:3]
        ...
        gdData['a_position'][:, 0] = np.tile(
            [-50, 50], len(dList))    # [1,0.99]
        gdData['a_position'][:, 1] = np.repeat(
            np.array(dList), 2)     # 2: each grid line need 2 points
        gdData['a_color'] = np.tile(QColor(QtCore.Qt.blue).getRgbF()[
                                    :3], (len(dList) * 2, 1))
    ...
    def on_draw(self, event):
        ...
        if self.enableDrawing:
            try:
                ...
                QtGui.QMessageBox.question(
                    None, 'Error', errorMsg, QtGui.QMessageBox.Ok)
                return
            """
            # labels
    ...
    def on_mouse_release(self, event):
        ...
        if v != False:
            ...
            self.LT, self.RB = v
            # save this new values for selection to zoom or pass to support
            # window
    ...
    def trimData(self, D):
        ...
        # do it for trimData() only, if other parts have problem, will consider
        # changing later
        ...
        for ch in self.control.channels:
            ...
            for i in range(startStatId, endStatId + 1):
                # self.control.metadata[i+orgStartId]['seq']
                statId = i + orgStartId
                if not self.control.channelCkbs[ch].isChecked() \
                ...
                if aSize > 0:
                    ...
                    # choose to add at the top or end of the list depend on the
                    # time direction
                    if self.control.upRbtn.isChecked():
                        if addLT:
    ...
    def _findTrimKeepList(self, i, timeVals, timeTop, timeBot):
        ...
        try:
            ...
            trimKeepList = np.where(
                (timeTop <= timeVals) & (timeVals <= timeBot))[0]
            ...
            if aSize == 0 \
                ...
                # this may happen because of the simplification cut off the
                # data approx to avg
                aSize = 2
                addLT = True
                addRB = True
    ...
    #    cut off data outside the selected section
    # change pans, scale to fit the selected section into the displaying
    # window
    ...
    def update_scale(self):
        ...
        self.limList = (
            self.canvScaleV * (self.control.statLimitList[L1:L2] + self.panV) + 1) * self.width * 0.5
    ...
    # Author: Lan
    # def: _zoomTo() 201511
    # change pans, scale to fit Left-Top, Right-Bottom positions into the
    # displaying window
    ...
    def onRight(self, evt):
        ...
        newLT['sentVal'] = LT['sentVal'] - \
            float(self.parent.distance.text()) * \
            1000 * 2 / self.control.totalSize
    ...
    # zoom to the new section which horizontally zoom out self.parent.distance
    # each side
    ...
    # zoom to the new section which horizontally zoom in self.parent.distance
    # each side
    ...
    # zoom to the new section which vertically zoom in self.parent.time each
    # side
    ...
    # zoom to the new section which vertically zoom out self.parent.time each
    # side
        ...
        if event._button == 2:
            ...
            for d in dataList:
                ...
                if count < len(self.parent.statSelectors):
                    ...
                    # 2. too many stations will overlap each other so usere
                    # can't see all anyway
                ...
                if control.correctionCkb.isChecked() or control.vel is not None:
                ...
                if control.stationSpacingUnknownCkb.isChecked():
                    ...
                    v = (
                        str(control.PH5Info['distanceOffset'][d['statId']]), 'm')
                info += "\nDistanceOffset: %s (%s)" % v

                info += "\nClockDriftCorrection: %s (ms)" % str(
                    statData['clockDriftCorr'])
    ...
    def locateDataPoint(self, x, y, getInfo=False):
        ...
        k = np.where((self.limList[:, 0] <= x) & (self.limList[:, 1] >= x) & (
        ...
        if not getInfo:
            if len(k[0]) > 0:
                ...
                if x <= minLim:
                    ...
                    # when x is inside limit but not in any limList, use the
                    # next greater available
                    k = np.where(self.limList[:, 0] > x)[0]
                    if self.control.PH5Info['up'] == True:
                        indeces = [k.min()]
                    else:
                        indeces = [k.max()]
        ...
        for i in indeces:
            ...
            # sentTimeVal: timeVal in range (-1,1) that has been sent to canvas
            # to draw
            sentTimeVal = ((2. * y / self.height - 1) /
                           self.canvScaleT - self.panT)
            ...
            dispTimeVal = (direct * sentTimeVal + 1) / \
                self.control.scaleT + self.control.minT
            ...
            valCenter = (self.control.metadata[statId]['minmax']
                         [1] + self.control.metadata[statId]['minmax'][0]) / 2
    ...
    def pr(self):
        ...
        vals = PrintSaveParaDialog.print_save(
            self, self.printType, 'inch', (paperRect.width(), paperRect.height()))
        if vals is None:
            return
    ...
    def save2file(self, saveType):
        ...
        fname = dialog.getSaveFileName(
            self.parent, 'Save As', '', '*.%s' % fileformat)
        if fname == '':
            return
        ...
        start = time.time()
        self.control.PH5Info['numOfStations']
        # clear the old figure
        plt.clf()
        fig = plt.figure(1, dpi=100)
        # if fileformat=='svg':
        ...
        try:
            ...
            pass
        # save figure with dpi=100. The dpi and w, h has been tried to get the
        # right size for the file
        if fileformat == 'svg':
    ...
    def quickRemove(self, ch, statId, removedStatus, c=0):
        ...
        if canvId < 0 or canvId >= len(self.data[ch]):
            return 2  # print "quickRemove return 2"; return 2
        aSize = len(self.data[ch][canvId]['a_index'])
        if removedStatus:
            self.control.PH5Info['quickRemoved'][ch][statId] = deepcopy(
                self.data[ch][canvId]['a_color'][0])
...
class PlottingPanel(QtGui.QMainWindow):
    def __init__(self, control, title, x, y, w, h, isMainPlot=True):
        ...
        self.resetZPBtn = resetZPBtn = QtGui.QPushButton(
            'Reset Zoom/Pan', mainCommandPanel)
        ...
        self.undoQuickRemovedBtn = undoQuickRemovedBtn = QtGui.QPushButton(
            'Undo QuickRemove', mainCommandPanel)
        ...
        if isMainPlot:
            self.deepRemovedBtn = deepRemovedBtn = QtGui.QPushButton(
                'DeepRemove', mainCommandPanel)
...
class PH5Visualizer(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.loiEventGui = ArrayGui(
            self, ESType='EVENT_LOI')  # LOI: lack of info
    ...
    def onDevelopeSegy(self):
        ...
        if str(self.mainControl.velocityCtrl.text()).strip() in ['0', '']:
            ...
            options['offset'] = '-O %f' % float(
                self.mainControl.offsetCtrl.text())

        if self.submitGui == 'STATION':
            print "shot gather"
            #options['array'] = '-a %s' % self.selectedArray['arrayId']
            options['stations'] = '--station_list %s' % ','.join(
        ...
        p = Popen(cmdStr % options, shell=True, stdin=PIPE,
    ...
    def onFile(self):
        ...
        except ph5_viewer_reader.PH5ReaderError as e:
            msg = e.message + \
        ...
        if PH5Object.graphExperiment is not None:
            self.graphName = "%s %s" % (
                PH5Object.graphExperiment['experiment_id_s'], PH5Object.graphExperiment['nickname_s'])
        ...
        # close all opened files and remove PH5Object when done to save
        # resources
        self._closePH5(PH5Object)
        #print "begin testing"
        #PH5Object = ph5_viewer_reader.PH5Reader()
...
class MainControl(QtGui.QMainWindow):
    ...
    def initConfig(self):
        ...
        try:
            ...
            for line in lines:
                l = line.split(":")
                if l[0] == 'addingInfo':
                    self.fileConf['addingInfo'] = l[1].strip()
                elif l[0] == 'hLabel':
                    self.fileConf['hLabel'] = l[1].strip()
                elif l[0] == 'vLabel':
                    self.fileConf['vLabel'] = l[1].strip()
                elif l[0] == 'gridColor':
                    self.fileConf['gridColor'] = l[1].strip()
                # elif l[0] == 'pointColor': self.fileConf['pointColor'] =
                # l[1].strip()
                elif l[0] == 'patternSize':
                    self.fileConf['patternSize'] = int(l[1].strip())
                elif l[0] == 'plotThick':
                    self.fileConf['plotThick'] = float(l[1].strip())
                elif l[0] == 'gridThick':
                    self.fileConf['gridThick'] = float(l[1].strip())
                elif l[0] == 'abnormalColor':
                    self.fileConf['abnormalColor'] = l[1].strip()
                elif 'plotColor' in l[0]:
                    # plotColor1: 1 is channel, when channel index changed,
                    # create new list
                    if ncolor != l[0][-1]:
                        ncolor = l[0][-1]
                        colorChan += 1
                    ...
                    self.fileConf['showAbnormalStat'] = True if l[1].strip(
                    ) == 'True' else False
        ...
        if self.fileConf['patternSize'] < self.defaultConf['patternSize']:
            # if new patternSize is smaller, need to trim the extra value
            # assigned before
            ...
            # if new patternSize is smaller, some missed channel need to be
            # filled up
        ...
        for ch in range(len(self.channels)):
            if ch < len(self.conf['plotColor']):
                self.channelCkbs[self.channels[ch]].setStyleSheet(
                    "QWidget { background-color: %s }" % QColor(self.conf['plotColor'][ch][0]).name())
            else:
                self.channelCkbs[self.channels[ch]].setStyleSheet(
                    "QWidget { background-color: %s }" % QColor(self.defaultConf['plotColor'][ch][0]).name())
    ...
    def initUI(self):
        ...
        gridBox.addWidget(QtGui.QLabel(
            'Ignore minor signal (0-20%)', self), 0, 0)
        ...
        self.stationSpacingUnknownCkb = QtGui.QCheckBox(
            'STATION SPACING UNKNOWN', self)
    ...
    # simplify affect keepList -> recalc both val and time, but don't need to
    # reread PH5data
    ...
    def getKeepList(self, val, simplFactor):
        ...
        for ch in self.channels:
            ...
            for i in range(staNo):
                ...
                # remove center
                V = val[ch][i][:samNo]
                a = np.where(abs(V - V.mean()) >
                             abs(V.max() - V.mean()) * simplFactor)[0]
                o = []
                d = V
    ...
    def getPH5Data(self, orgStartT, offset, timeLen, staSpc):
        ...
        # convert list to numpy array => check to see if can create numpy array
        y = PH5Object.data
        ...
        for chId in range(len(self.channels)):
            ...
            # try :
            ph5Valfile = np.memmap(
                PH5VALFILES[chId], dtype='float32', mode='w+', shape=(1, PH5Val.size))
            ph5Valfile[:] = PH5Val[:]
            del ph5Valfile
            # except :
            # pass
    ...
    def createVal(self, createFromBeg=True, appNewSimpFactor=False):
        ...
        if self.PH5Info['numOfDataStations'] > 1:
            # need to subtract 1 because each offset is only the center of each
            # plot, not the min side or max side
        ...
        self.distanceGridIntervalSB.setRange(
            int(self.totalSize / (1000 * 50)), int(self.totalSize / 1000))
        ...
        if not self.normalizeCkb.isChecked():
            ...
            for ch in self.channels:
                try:
                    maxP2Plist.append(max([abs(m['minmax'][0] - m['minmax'][1]) for m in self.metadata
                                           if m is not None and self.PH5Info['LEN'][ch][self.metadata.index(m)] != 0
            ...
            self.scaleVList = [scaledSizeV4aplot /
                               float(maxP2P)] * self.PH5Info['numOfStations']
        ...
        for i in range(len(self.metadata)):
            ...
            if self.metadata[i] is None:
                # this info won't be used. just add to keep index
                self.scaleVList.append(1)
                # this info won't be used. just add to keep index
                zeroList.append(0)
                continue
            if self.normalizeCkb.isChecked():
                # difference bw min and max (peak to peak)
                p2p = abs(self.metadata[i]['minmax']
                          [1] - self.metadata[i]['minmax'][0])
            ...
            centerVal = (self.metadata[i]['minmax']
                         [1] + self.metadata[i]['minmax'][0]) / 2
            zeroList.append(self.statLimitList[i].mean(
            ) - centerVal * self.scaleVList[i])
            ...
            for i in range(len(self.metadata)):
                ...
                if i in self.PH5Info['deepRemoved'][ch] \
                   or self.PH5Info['LEN'][ch][i] == 0:  # (i,ch) in self.PH5Info['zerosList']:
                    ...
                    if self.distance2AvgSB.value() > 0:
                        ...
                        # self.ph5val.append(val[i][self.keepList[i]])    # for
                        # double-check
                ...
                if i % 10 == 0:
                    showStatus("Step3: Prepare drawing data",
                               "%s/%s" % (i, len(val[ch])))
                PH5Val[ch][i] = PH5Val[ch][i] * \
                    self.scaleVList[i] + zeroList[i]
    ...
    def processData(self):
        ...
        realRange = self.PH5Info['numOfSamples'] * \
            self.PH5Info['interval'] / 1000
...
class Properties(QtGui.QDialog):
    ...
    def initUI(self):
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "AddingInfo to GraphicName", propertiesPanel), 0, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Horizontal Label", propertiesPanel), 2, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Vertical Label", propertiesPanel), 3, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Pattern Size", propertiesPanel), 4, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Trace Thickness", propertiesPanel), 5, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Grid Thickness", propertiesPanel), 7, 0, QtCore.Qt.AlignRight)
        ...
        propertiesGrid.addWidget(QtGui.QLabel(
            "Abnormal Station(s)", propertiesPanel), 8, 0, QtCore.Qt.AlignRight)
    ...
    # Author: Lan
    # def: eventFilter(): 20151022
    # using baloon tooltip to help user understand the use of the widget (only
    # the one install event filter)
    def eventFilter(self, object, event):
        if not self.helpEnable:
            return False
        if event.type() == QtCore.QEvent.Enter:
    ...
    def onApply(self, evt):
        ...
        errorItm = None
        try:
            self.parent.fileConf['patternSize'] = int(
                self.patternSizeText.text())
        except ValueError:
            errorItm = "Pattern Size"
            expectType = "an integer"

        try:
            self.parent.fileConf['plotThick'] = float(
                self.plotThickText.text())
        except ValueError:
            errorItm = "Plot Thickness"
            expectType = "a float"

        try:
            self.parent.fileConf['gridThick'] = float(
                self.gridThickText.text())
        except ValueError:
        ...
        self.parent.fileConf['gridColor'] = self.gridColBtn.palette().color(
            1).name()
        self.parent.fileConf['abnormalColor'] = self.abnormalColBtn.palette().color(
            1).name()
    ...
    # when click on the button, pop-up QColorDialog for user to select one
    # color, change color of the button to the selected color
    def onChangeColor(self, evt):
        ...
        for ch in range(len(self.plotColBtns)):
            if self.sender() in self.plotColBtns[ch]:
                ...
                if bIndex == 0:
                    # allColorBtns are set to the color of the first button in that channel
                    # so when the first button's color is changed, the
                    # allColorBtns for that channel should be changed
    ...
    def setConf(self):
        ...
        if 'gridColor' in self.conf:
            self.gridColBtn.setStyleSheet(
                "QWidget { background-color: %s }" % QColor(self.conf['gridColor']).name())

        if 'abnormalColor' in self.conf:
            self.abnormalColBtn.setStyleSheet(
                "QWidget { background-color: %s }" % QColor(self.conf['abnormalColor']).name())
    ...
    # add color buttons according to pattern; any extra button, use default
    # color
    def updatePlotColorButton(self, pSize):
        ...
        self.plotColBtns = []
        for ch in range(len(self.parent.channels)):
            # allColorBtns are set to the color of the first button in that
            # channel
            if ch < len(self.conf['plotColor']):
                self.allColorBtns[ch].setStyleSheet(
                    "QWidget { background-color: %s }" % QColor(self.conf['plotColor'][ch][0]).name())
                ...
                self.allColorBtns[ch].setStyleSheet(
                    "QWidget { background-color: %s }" % QColor(self.parent.defaultConf['plotColor'][ch][0]).name())
            ...
            for i in range(pSize):
                ...
                if ch < len(self.conf['plotColor']):
                    if i < self.conf['patternSize']:
                        cb.setStyleSheet(
                            "QWidget { background-color: %s }" % QColor(self.conf['plotColor'][ch][i]).name())
                    else:
                        cb.setStyleSheet(
                            "QWidget { background-color: %s }" % QColor(self.conf['plotColor'][ch][0]).name())
                else:
                    cb.setStyleSheet("QWidget { background-color: %s }" % QColor(
    ...
    # Author: Lan
    # def: onUpdate():201409
    # reset the color buttons according to the number entered in
    # patternSizeText
    def onUpdate(self):
...
class ArrayGui(QtGui.QWidget):
    ...
    def setNotice(self, graphName):
        self.statusCtrl.setText(
            "Graph Name is '%s'. Click on Properties in Control tab to change name of the graph" % graphName)
...
class ES_Gui(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        # event form need shotCtrls (shotline) which should be radio button to
        # allow selecting only one shotline at a time
        self.headBox = QtGui.QHBoxLayout()
        mainVbox.addLayout(self.headBox)
        if self.ESType in ['STATION', 'EVENT_LOI']:
            # if self.parent.__class__.__name__ == 'ES_Gui':  # => this is
            # submit form
        ...
        if not self.submitType or self.ESType == 'EVENT_LOI':
            ...
            mainVbox.addWidget(QtGui.QLabel(
                "Array Info: %s sps || %s - %s" % v))
    ...
    def onSubmit(self, evt):
        ...
        if self.ESType == 'EVENT':
            ...
            # use orderedStationIds to have the station sorted from the reader
        ...
        if self.ESType != 'EVENT_LOI':
            ...
            control.upperTimeLen = self.array['pickupT'] - \
                self.array['deployT']
    ...
    def setEvents(self):
        ...
        for e in self.PH5View.events['events']:
            if e['eStop'] < self.array['deployT'] or e['eStart'] > self.array['pickupT']:
                continue
            ...
            self.addLabel(self.ESGrid, timedoy.epoch2passcal(
                e['eStart']), lineSeq, 3)
    ...
    # select one event => list of stations for the array of this GUI to will
    # show up
    def onSelectEvent(self, state):
        # only one event in the selectedEvents
        self.PH5View.selectedEvents = [
            e for e in self.array['events'] if e['eventRbtn'] == self.sender()]
    ...
    # Author: Lan
    # def: onSelectEvent():201612
    # select one station => list of events of which times belong to the time
    # of this GUI's array
    ...
    def onSelectEventRange(self, state):
        ...
        # check with shift, add the range from this event to the closest one to
        # selectedEventChks
    ...
    def onSelectStationRange(self, state):
        ...
        ) == False:  # if uncheck, remove that station from selectedStationChks
        ...
        # check with shift, add the range from this station to the closest one
        # to selectedStationChks
    ...
    def eventFilter(self, object, event):
        ...
        if event.type() == QtCore.QEvent.Enter:
            ...
            if object.__class__.__name__ == 'QRadioButton' \
                ...
                QtGui.QToolTip.showText(self.scrollPanel.mapToGlobal(
                    QtCore.QPoint(P.x(), P.y() + 20)), self.EXPL[object])
            else:
                QtGui.QToolTip.showText(self.mapToGlobal(
                    QtCore.QPoint(P.x(), P.y() + 20)), self.EXPL[object])
...
def startapp():
    ...
    QtCore.QObject.connect(application, QtCore.SIGNAL(
        "focusChanged(QWidget *, QWidget *)"), changedFocusSlot)
