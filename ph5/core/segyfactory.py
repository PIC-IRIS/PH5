...
class Ssegy:
    ...
    def write_trace_header(self, fd):
        ...
        try:
            #print os.stat (fd.name)[6]
            n = fd.write(self.extended_header.get()[:60])
            fd.flush()
            #print os.stat (fd.name)[6]; print
        except Exception as e:
            raise SEGYError(
                "Failed to write extended portion of SEG-Y trace header: {0}".format(e.message))
    ...
    def set_reel_header(self, traces):
        ...
        try:
            ...
            raise SEGYError(
                "Possible overflow in SEG-Y reel header: {0}".format(e.message))
    ...
    def set_trace_header(self):
        ...
        try:
            tra['traceWeightingFactor'] = int(
                math.log(self.response_t['bit_weight/value_d'] / mult, 2) + 0.5)
        except ValueError:
        ...
        try:
            ...
            raise SEGYError(
                "Possible overflow in extended portion of trace header: {0}".format(e.message))
...
def write_segy_hdr(trace, fd, sf, num_traces):
    ...
    if len(data) > MAX_16 and sf.break_standard == False:
        errors.append("Warning: Data trace too long, %d samples, truncating to %d" % (
            len(data), MAX_16))
...
def calc_red_vel_secs(offset_t, red_vel):
    ...
    if offset_t is None:
        errors.append(
            "Warning: No geometry for station. Reduction velocity not applied.")
        return 0., errors

    if offset_t['offset/units_s'] != 'm':
        errors.append(
            "Warning: Units for offset not in meters! No reduction velocity applied.")
        return 0., errors

    #   m / m/s = seconds
    try:
        secs = abs(offset_t['offset/value_d']) / (red_vel * 1000.)
        errors.append("Applying a reduction velocity of {0:5.3f} seconds (Shot: {1}, Receiver: {2})".format(
