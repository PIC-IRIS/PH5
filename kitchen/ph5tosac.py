...
class lopttime :
    ...
    def epoch (self) :
        ...
        try :
            ...
            sc = self.sc + (self.ms / 1000.)
            return TimeDOY.yrdoyhrmnsc2epoch (self.year, self.doy, self.hr, self.mn, self.sc, self.ms * 1000, fepoch=True)
        except Exception, e :
...
def get_args () :
    ...
    global START_TIME, STOP_TIME, UTM, EXT, DEPLOY_PICKUP, BREAK_STANDARD, EVENT_NUMBER, STA_LIST
...
def cut (start, stop, Das_t, time_t, Response_t, Receiver_t, sf) :
    ...
    if DECIMATION :
        ...
        #print "Samples Read:", samples_read, "Length Points All:", sf.length_points_all
...
def process_event () :
    ...
    for a in arrays :
        ...
        #tdoy = TimeDOY.TimeDOY (epoch=start_epoch)
        
        base = "%s_%s_%s_%s" % (nickname, a, TimeDOY.epoch2passcal (start_epoch, sep='_'), chan_name)
...
def process_all () :
    ...
    for das in dass :
        ...
        for das_t in Das_t.rows :
            ...
            #tdoy = TimeDOY.TimeDOY (epoch=cut_start)
            
            base = "%s_%s_%s_%s_00001.SAC" % (nickname, TimeDOY.epoch2passcal (cut_start), das, chan_name)
            doy = base.split (':')[1]
